<x-admin-layout :page-title="$pageTitle" :breadcrumb="$breadcrumb">
    <div class="space-y-6">
        ...
        <form method="GET" action="{{ route('admin.products.index') }}" class="rounded-3xl border border-rose-100 bg-white p-5 shadow-sm">
            <div class="grid gap-3 md:grid-cols-5">
                <x-text-input name="search" :value="$filters['search']" placeholder="Search name/skill/material" class="block w-full" />
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.products.index') }}" class="rounded-xl border border-rose-100 px-4 py-2 text-sm font-semibold text-stone-600 hover:bg-rose-50">Reset</a>
                </div>
            </div>
        </form>

        <div class="flex justify-end">
            <a href="{{ route('admin.products.create') }}" class="inline-flex rounded-xl bg-rose-900 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-800">Add Product</a>
        </div>
        ...
        <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            @forelse ($products as $product)
                ...
                <article class="overflow-hidden rounded-3xl border border-rose-100 bg-white shadow-xl shadow-rose-100/40">
                    ...
                    @else
                        <div class="flex h-48 items-center justify-center bg-[#fbf7f0] text-sm font-semibold text-stone-500">No image uploaded</div>
                    @endif
                    <div class="p-5">
                        <div class="flex flex-wrap gap-2">
                            <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $product->is_active ? 'bg-emerald-100 text-emerald-900' : 'bg-stone-100 text-stone-600' }}">{{ $product->is_active ? 'Active' : 'Inactive' }}</span>
                            @if ($product->is_featured)
                                <span class="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-900">Featured</span>
                            @endif
                        </div>
                        <h3 class="mt-3 text-base font-semibold text-stone-950">{{ $product->name }}</h3>
                        <p class="mt-1 text-xs text-stone-500">{{ $product->category?->name ?: 'Uncategorized' }}</p>
                        <p class="mt-2 text-sm text-stone-600">{{ $product->short_description ?: '-' }}</p>
                        <p class="mt-2 text-xs text-stone-500">{{ $product->skill_used ?: '-' }} @if($product->material) • {{ $product->material }} @endif</p>
                        <div class="mt-4 flex flex-wrap gap-2">
                            <a href="{{ route('admin.products.edit', $product) }}" class="rounded-xl border border-rose-100 px-3 py-1.5 text-xs font-semibold text-rose-900 hover:bg-rose-50">Edit</a>
            ...
            @empty
                <div class="rounded-3xl border border-dashed border-rose-200 bg-white p-8 text-sm text-stone-500 sm:col-span-2 xl:col-span-4">No products found.</div>
            @endforelse
        </section>
