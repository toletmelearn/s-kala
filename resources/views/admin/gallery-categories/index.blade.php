<x-admin-layout :page-title="$pageTitle" :breadcrumb="$breadcrumb">
    <div class="space-y-6">
        ...
        <div class="flex justify-end">
            <a href="{{ route('admin.gallery-categories.create') }}" class="inline-flex rounded-xl bg-rose-900 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-rose-900/15 hover:bg-rose-800">
                Add Category
            </a>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-rose-100">
                    ...
                    <tbody class="divide-y divide-rose-100">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-wrap gap-2">
                                        <a href="{{ route('admin.gallery-categories.edit', $category) }}" class="rounded-xl border border-rose-100 px-3 py-1.5 text-xs font-semibold text-rose-900 hover:bg-rose-50">Edit</a>
                                        <form method="POST" action="{{ route('admin.gallery-categories.toggle-status', $category) }}">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <form method="POST" action="{{ route('admin.gallery-categories.destroy', $category) }}" onsubmit="return confirm('Delete this category?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-10 text-center text-sm text-stone-500">No categories added yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
